...
namespace FloorMod
{
    public class Program
    {
        ...
        public static int Main(string[] args)
        {
            ...
            if (success)
            {
                Console.WriteLine($"FloorMod succeeded! You may close this window now.");
            }
            else
            {
                Console.WriteLine($"FloorMod failed!");
            }
            Console.ReadKey();
            FreeConsole();
            return success ? 0 : 1;
        ...
        private static string GetVar(string ini, string section, string var)
        {
            ...
            {
                Console.WriteLine($"floorconfig.ini variable {var} in section [{section}] not found or missing");
            }
            return result;
        }

        enum Effect { Poison, Fire, Terror, Enfeeble }

        enum Start { None, Healthy, Delayed }

        private static void RunMod(string configDir, string inDir, string outDir)
        {
            string ini = new FileInfo(Path.Combine(configDir, "floorconfig.ini")).FullName.ToString();
            if (!File.Exists(ini))
            {
            ...
            {
                string[] searchOrder = { @"..\oo2core_6_win64.dll", @"..\..\oo2core_6_win64.dll", @"C:\Program Files (x86)\Steam\steamapps\common\Sekiro\oo2core_6_win64.dll" };
                ...
                if (!File.Exists("oo2core_6_win64.dll"))
                {
                    throw new Exception("Oodle not found. Copy oo2core_6_win64.dll from Sekiro.exe directory");
                }
            }

            GameEditor game = new GameEditor(FromGame.SDT);
            game.Spec.GameDir = inDir;
            game.Spec.DefDir = "Defs";
            Dictionary<string, PARAMDEF> defs = game.LoadDefs();
            Dictionary<string, PARAM> param = game.LoadParams(defs);
            if (!param.ContainsKey("SpEffectParam") || !param.ContainsKey("HitMtrlParam"))
            {
            ...
            if (!Enum.TryParse(startStr, true, out Start start))
            {
                throw new Exception($"Unsupported startMod {startStr}");
            }

            bool applyEverywhere = GetVar(ini, "mod", "applyEverywhere") == "1";
            ...
            // 4066: dummy speffect to copy to 4067 for player
            PARAM.Row dummySp = copySpEffect(5020, 4066, "trigger");
            dummySp["effectEndurance"].Value = 0f;

            // 4067: rate-limiting speffect
            PARAM.Row rateSp = copySpEffect(4002, 4067, "rate");
            ...
            param["SpEffectParam"][4067]["cycleOccurenceSpEffectId"].Value = 4068;
            ...
            if (!dryrun) game.OverrideBnd($@"{inDir}\param\gameparam\gameparam.parambnd.dcx", Path.GetDirectoryName(path), param, f => f.Write());
